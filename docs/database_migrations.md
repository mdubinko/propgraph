...
with PropertyGraph("my_database.db") as graph:
    current_version = graph.prop("schema_version")  # Returns int, e.g., 1
```
...
def migrate_database(graph: PropertyGraph, from_version: int, to_version: int):
    ...
    while current_version < to_version:
        ...
        if current_version in migrations and next_version in migrations[current_version]:
            ...
            graph.set_prop("schema_version", next_version)
            graph.commit()
    ...
    for node in graph.nodes("old_file_type"):
        ...
        for edge in graph.iter_edges():
            if edge.src_id == node.node_id:
                graph.add_edge(new_node, edge.edge_type, edge.dst_id, **dict(edge.iter_props()))
            elif edge.dst_id == node.node_id:
                graph.add_edge(edge.src_id, edge.edge_type, new_node, **dict(edge.iter_props()))
...
def convert_properties(old_node):
    ...
    props = dict(old_node.iter_props())
...
class PropertyGraph:
    ...
    def _migrate_if_needed(self):
        ...
        current = self.prop("schema_version") or 1
...
See `src/propgraph.py` for the base PropertyGraph class that supports this migration system.
...
def test_migration():
    ...
    # Create v1 database with test data
    with PropertyGraph(":memory:") as graph_v1:
        graph_v1.set_prop("schema_version", 1)
        test_node = graph_v1.add_node("file", path="/test.py")
        ...
        # Verify migration worked
        assert graph_v1.prop("schema_version") == 2
        migrated_node = list(graph_v1.nodes("source_file"))[0]
        assert migrated_node.prop("file_path") == "/test.py"
```
